getPaths = (x, m, options) ->
  ###
  x: top start for path discovery in matrix
  ...
  options:
    ...
    debug: return all directions and weights obtained
  ###
  ...
  getRange = (x, step) =>
    ...
    # x range limits for current step
    range_start = x-step
    range_end = x+step
...
module.exports.getMatrixMinWeight = (matrix) ->
  minWeight = null
